...
class Game:
    def __init__(self):
        ...
        pr = lambda n, rad: load_tex(img, n, max(8, int(2 * rad * ds)))
        self.tex_knight = pr("knight.png", config.PLAYER_RADIUS)
        self.tex_sword = load_tex(img, "sword.png", max(12, int(20 * ds)))
        self.tex_enemy = {rad: pr("enemy.png", rad) for rad in (9, 12, 18)}
        self.tex_bullet = pr("bullet.png", config.PROJECTILE_RADIUS)
        self._start_bgm()
        self.reset()
